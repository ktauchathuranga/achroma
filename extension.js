...
class AchromaIndicator extends PanelMenu.Button {
    _init(extensionObject) {
        ...
        this._isActive = this._settings.get_boolean('is-previewing') ? false : false; // Start safe
    ...
    _applyEffectData(profile) {
        ...
        if (profile.tint && profile.tint.a > 0) {
            const t = profile.tint;
            const colorEffect = new Clutter.ColorizeEffect({
                tint: new Clutter.Color({ red: t.r, green: t.g, blue: t.b, alpha: t.a })
            });
            uiGroup.add_effect_with_name(EFFECT_TINT, colorEffect);
        }
    }
